...
function destroy(container)
	if container.window then
		...
		if parent ~= nil then
			parent:removeWidgetFromScheduler(container.window)
		end
...
function onContainerOpen(container, previousContainer)
  ...
  if previousContainer then
    ...
    containerWindow = g_ui.createWidget('ContainerWindow', modules.game_interface.getContainerPanel())
		containerWindow.containerWindow = true
  ...
  local cellSize = layout:getCellSize()
	...
	containerWindow:setContentMaximumHeight((cellSize.height + layout:getCellSpacing()) * layout:getNumLines() + containerPanel:getPaddingTop() + containerPanel:getPaddingBottom())
	-- containerWindow:setOriginHeight(containerWindow:getMinimumHeight())

	if container:hasPages() then
		local height = containerWindow.miniwindowScrollBar:getMarginTop() + containerWindow.pagePanel:getHeight() + 23
  ...
  if not previousContainer then
    ...
    local filledLines = math.max(math.ceil(container:getItemsCount() / layout:getNumColumns()), 1)
		containerWindow:setContentHeight(filledLines * cellSize.height + filledLines * 3)
  end

  containerWindow:setup()
	if containerWindow:getParent() and containerWindow:getParent():isExplicitlyVisible() then
		containerWindow:getParent():saveChildren()
	end
end

function onContainerClose(container)
	if container.window then
		container.window:saveParentIndex(nil, nil)
	end

	local parentToReload

	if container.window and container.window:getParent() and container.window:getParent():getClassName() == "UIMiniWindowContainer" then
		parentToReload = container.window:getParent()
	end

	destroy(container)

	if parentToReload then
		parentToReload:reloadChildReorderMargin()
	end
end
