...
function UIResizeBorder:getMaximum()
	if self:getParent() and self:getParent():getParent() and self:getParent():getClassName() == "UIMiniWindow" and self:getParent():isFromMainPanel() then
		local miniWindow = self:getParent()
		local mainPanel = miniWindow:getParent()

		return modules.game_interface.getFirstRightSidePanel():getHeight() - mainPanel:getHeight() + miniWindow:getHeight() - 128
	end
