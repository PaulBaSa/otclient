...
local GameServerUpdatePartyMemberPosition = 46
function getMinimap()
	return minimapWindow
end

function init()
	minimapWindow = g_ui.loadUI("minimap", modules.game_interface.getMainRightPanel())
	minimapWindow.forceHideScrollBar = true

	minimapWindow:getChildById("miniwindowScrollBar"):hide()
	minimapWindow:setContentMinimumHeight(64)
	minimapWindow:setContentMaximumHeight(512)

	local minimapHeight = g_settings.getNumber("minimapHeight")

	if minimapHeight > 0 then
		minimapWindow:setHeight(minimapHeight)
	end
...
function onGeometryChange(self, oldRect, newRect)
	modules.game_interface.onGeometryChange(self, oldRect, newRect)
end

function reloadResizer()
	if minimapWindow.ignoreResizerRecursive then
		return
	end
	...
	minimapWindow.ignoreResizerRecursive = true

	resizeBorder:reloadHeight()

	minimapWindow.ignoreResizerRecursive = false
end
...
function parseUpdatePartyMemberPosition(protocol, msg)
  ...
  if displayPartyMembersOnMinimap then
