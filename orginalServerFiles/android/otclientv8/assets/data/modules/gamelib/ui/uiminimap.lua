...
function UIMinimap:clearCrossPartyPosition()
	if self.partyCross then
		for _, v in pairs(self.partyCross) do
			v:destroy()
		end
...
function UIMinimap:getVocationIconClip(vocationId)
	local clip = '0 0 19 19'
...
function UIMinimap:setCrossPartyPosition(name, vocationId, pos)
  if not self.partyCross then
    self.partyCross = {}
  end

  local cross = self.partyCross[name]

  -- Verificar se o jogador jÃ¡ tem um marcador; se nÃ£o, criar um novo.
  if not cross then
    cross = g_ui.createWidget('MinimapPartyCross', self)
    cross:getChildById('name'):setText(name)
    cross:setWidth(cross:getChildById('name'):getWidth())
    self.partyCross[name] = cross

    local creatureWidget = cross:getChildById('cross')
  ...
  -- Configurar o Ã­cone de acordo com a vocaÃ§Ã£o, mas sem afetar a lÃ³gica de exibiÃ§Ã£o.
  local creatureWidget = cross:getChildById('cross')
  creatureWidget:setIconClip(self:getVocationIconClip(vocationId))

  -- Atualizar a posiÃ§Ã£o no minimapa
  pos.z = self:getCameraPosition().z
  cross.pos = pos
  self:centerInPosition(cross, pos)
end
