...
local function lock(dontSave)
	local lockButton = inventoryWindow:recursiveGetChildById("lockButton")
	...
	if not dontsave then
		inventoryWindow:setSettings({
			locked = true
		})
	end

	signalcall(inventoryWindow.onLockChange, inventoryWindow)
end

local function unlock(dontSave)
	local lockButton = inventoryWindow:recursiveGetChildById("lockButton")
...
function init()
  ...
  connect(g_game, { onGameStart = refresh })
	...
	inventoryWindow = g_ui.loadUI("inventory", modules.game_interface.getMainRightPanel())
	inventoryWindow.forceHideScrollBar = true
	inventoryWindow.ignoreExpandingHeight = true
	inventoryWindow:getChildById("miniwindowScrollBar"):hide()
	inventoryWindow:disableResize()
	inventoryWindow:setContentMinimumHeight(175)
	inventoryWindow:setContentMaximumHeight(175)
...
function online()
	if inventoryWindow.minimizeButton:isOn() then
...
function onClickMinimizeButton(widget)
	widget:getParent():getParent().minimizeButton:onClick()
end
