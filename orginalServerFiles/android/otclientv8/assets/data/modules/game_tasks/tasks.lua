...
function create()
  -- Verificar se os botões já existem antes de criá-los
  if not openTasksButton then
    openTasksButton = modules.client_topmenu.addLeftButton("openTasksButton", tr("Open Tasks Panel"), "/images/topbuttons/task", toggleTasksPanel)
    openTasksButton:setOn(true)
  end

  if not trackerButton then
    trackerButton = modules.client_topmenu.addLeftButton("trackerButton", tr("Tasks Tracker"), "/images/topbuttons/battle", toggleTracker)
...
function toggleTasksPanel()
  ...
  if protocolGame then -- # Kow
    local TasksPanel = tasksWindow.tasksList
    if TasksPanel and #TasksPanel:getChildren() <= 0 then
      trackerWindow.contentsPanel.trackerPanel:destroyChildren()
  	  protocolGame:sendExtendedOpcode(OPCODE, json.encode({action = "toggleTask"}))
    end
end
...
function onTaskUpdate(data)
  ...
  -- Verifique se o taskId é válido e existe na tabela tasks
  local task = tasks[data.taskId]
  if not task then
    g_logger.error("[Tasks] Task not found for taskId: " .. tostring(data.taskId))
    return
  end
