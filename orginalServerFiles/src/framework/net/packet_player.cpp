...
#endif
     if (!f.is_open()) {
        g_logger.error("Erro ao abrir replay: " + file);
        return;
    } else {
        g_logger.info("Replay carregado com sucesso: " + file);
    }

    std::string type, packetHex;
    ticks_t time;
    while (f >> type >> time >> packetHex) {
        g_logger.info("Lendo pacote do replay: " + type + " | Tempo: " + std::to_string(time) + " | Hex: " + packetHex);
